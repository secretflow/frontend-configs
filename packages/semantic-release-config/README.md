...
```bash
$ npm install --save-dev semantic-release @secretflow/semantic-release-config
```
...
```json
{
  "extends": "@secretflow/semantic-release-config"
}
```

We can also specify plugin options in `.releaserc.js` file:
```js
var config = require('@secretflow/semantic-release-config/config');
